  ...
  * The content rows of a space should not be available to non-members
  ...
  Background:
    ...
    And Document are added to the database:
      | $id | _space_id | source_local_id | _author_id | created    | last_modified | content_type          |
      | d1  | s1        | abc             | user1      | 2025/01/01 |    2025/01/01 | application/roam+json |
      | d2  | s1        | def             | user2      | 2025/01/01 |    2025/01/01 | application/roam+json |
      | d3  | s2        | ghi             | user3      | 2025/01/01 |    2025/01/01 | application/roam+json |
