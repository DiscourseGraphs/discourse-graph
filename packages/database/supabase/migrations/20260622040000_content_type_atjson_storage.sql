...
CREATE UNIQUE INDEX content_space_local_id_variant_idx
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM information_schema.columns
    WHERE table_schema = 'public'
      AND table_name = 'FileReference'
      AND column_name = 'content_type'
  ) THEN
    ALTER TABLE public."FileReference"
    ADD COLUMN content_type text GENERATED ALWAYS AS ('text/markdown') STORED;
  END IF;
END $$;
...
ALTER TYPE public.content_local_input
ADD ATTRIBUTE IF NOT EXISTS content_type text;
...
CREATE OR REPLACE FUNCTION public.upsert_content(
  v_space_id bigint,
  data jsonb,
  v_creator_id BIGINT,
  content_as_document boolean DEFAULT TRUE
)
