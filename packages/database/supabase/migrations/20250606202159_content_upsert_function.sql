...
CREATE OR REPLACE FUNCTION public.local_document_to_db_document(data public.document_local_input) RETURNS public."Document" LANGUAGE plpgsql STABLE AS $$
DECLARE
...
COMMENT ON FUNCTION public.local_document_to_db_documentIS 'utility function so we have the option to use platform identifiers for document upsert' ;


CREATE OR REPLACE FUNCTION public.local_content_to_db_content (data public.content_local_input) RETURNS public."Content" LANGUAGE plpgsql STABLE AS $$
DECLARE
...
COMMENT ON FUNCTION public.local_content_to_db_contentIS 'utility function so we have the option to use platform identifiers for content upsert' ;
...
BEGIN
  ...
  LOOP
    ...
    END IF;
    db_document := public.local_document_to_db_document(local_document);
...
BEGIN
  ...
  LOOP
    ...
    db_content := public.local_content_to_db_content(local_content);
    ...
    IF source_local_id(document_inline(local_content)) IS NOT NULL THEN
      db_document := local_document_to_db_document(document_inline(local_content));
      INSERT INTO public."Document" (
        space_id,
        source_local_id,
        url,
