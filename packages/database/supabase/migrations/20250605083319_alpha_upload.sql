-- These functions were created directly in supabase,
-- which was probably important as experimentation,
-- but best practice would be to go through a migration file.
-- fortunately we can create a migration file post-hoc with
-- supabase db pull
...
-- Note: this may be doable directly in postgrest
...
CREATE OR REPLACE FUNCTION public.alpha_upsert_discourse_nodes(p_space_name text, p_user_email text, p_user_name text, p_nodes jsonb)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
  ...
  SELECT id, platform_id INTO v_space_id, v_platform_id FROM "Space" WHERE name = p_space_name;
  IF NOT FOUND THEN
    RAISE WARNING 'Space not found for name: %', p_space_name;
    -- v_space_id will be NULL; execution continues but operations requiring it will be skipped.
  END IF;

  -- 2. Validate Platform ID (Optional: depending on strictness, could RAISE EXCEPTION if not found)
  IF v_platform_id IS NOT NULL THEN
    PERFORM id FROM "Platform" WHERE id = v_platform_id;
    IF NOT FOUND THEN
      RAISE WARNING 'Platform not found for id: % (associated with space: %)', v_platform_id, p_space_name;
      -- v_platform_id could be set to NULL or handled as per requirements
    END IF;
  END IF;

  -- 3. Get Person ID (Agent ID) or Create if not exists
  SELECT id INTO v_person_id FROM "Person" WHERE email = p_user_email;
  IF NOT FOUND THEN
    RAISE NOTICE 'Person with email % not found. Attempting to create.', p_user_email;
    IF p_user_name IS NULL OR trim(p_user_name) = '' THEN
      RAISE EXCEPTION 'Cannot create person with email % because p_user_name is missing or empty.', p_user_email;
    END IF;

    BEGIN
      -- Create corresponding Agent record first.
      -- IMPORTANT: Ensure you have an ENUM type for Agent.type (e.g., "AgentType")
      -- and a value like 'PERSON' within that ENUM. Adjust if your ENUM is named differently.
      INSERT INTO "Agent" (type)
      VALUES ('PERSON'::"AgentType") -- Replace "AgentType" with your actual ENUM name if different
      RETURNING id INTO v_person_id;

      -- Create Person record
      INSERT INTO "Person" (id, name, email)
      VALUES (v_person_id, p_user_name, p_user_email);

      RAISE NOTICE 'Successfully created Person with ID % (Agent ID), Name: %, Email: %.', v_person_id, p_user_name, p_user_email;
    EXCEPTION
      WHEN OTHERS THEN
        -- If creation fails, raise an exception to halt the process, as subsequent operations depend on a valid person.
        RAISE EXCEPTION 'Failed to create Person with email % and name %: %. SQLSTATE: %', p_user_email, p_user_name, SQLERRM, SQLSTATE;
    END;
  END IF;
  -- If v_person_id is still NULL here, it means the initial lookup failed AND the creation block
  -- somehow didn't execute or failed in an unhandled way (which it shouldn't due to EXCEPTION).
  -- For safety, though, the downstream checks for v_person_id IS NOT NULL are still valuable.


  -- 4. Process each node: Create/Update Document, Content, and Upsert Embedding
  FOR node IN SELECT * FROM jsonb_array_elements(p_nodes)
  LOOP
    DECLARE
      v_node_uid TEXT := node->>'uid';
      v_node_specific_document_id BIGINT;
      v_node_text TEXT := node->>'text';
      v_node_vector VECTOR; -- Assuming 'vector' type is defined
    ...
    BEGIN
      ...
      v_node_last_modified_at := COALESCE(v_node_last_modified_at, v_current_time);
      ...
      -- Proceed with Document/Content operations only if v_space_id and v_person_id were found/created.
      IF v_space_id IS NOT NULL AND v_person_id IS NOT NULL THEN
          ...
          IF v_node_specific_document_id IS NULL THEN
            ...
            VALUES (v_space_id, v_person_id, v_node_created_at, v_node_last_modified_at, v_node_uid, v_node_metadata)
        ...
        -- Proceed with Content operations only if Document was found/created.
        IF v_node_specific_document_id IS NOT NULL THEN
          INSERT INTO "Content" (
              text, scale, space_id, author_id, creator_id,
              document_id, source_local_id, metadata, created, last_modified
          )
          VALUES (
              v_node_text, 'document'::"Scale", v_space_id, v_person_id, v_person_id,
          ...
          END IF;
        END IF; -- end check for v_node_specific_document_id
      ELSE
        RAISE WARNING 'Skipping node UID % due to missing Space ID or Person ID. Space ID: %, Person ID: %', v_node_uid, v_space_id, v_person_id;
      END IF; -- end check for v_space_id and v_person_id
