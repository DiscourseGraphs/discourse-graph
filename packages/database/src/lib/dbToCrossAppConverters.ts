...
const getConceptMap = async ({
  ...
  return Object.fromEntries(
    (request.data || []).map(({ id, source_local_id, space_id }) => [
      id,
      space_id === spaceId || spaceId === undefined || spaceMap === undefined
        ? source_local_id
        : spaceUriAndLocalIdToRid(spaceMap[space_id]!, source_local_id),
    ]),
  );
};
...
// const reverseSpaceMap = (
//   spaceMap: Record<number, string>,
// ): Record<string, number> =>
//   Object.fromEntries(
//     Object.entries(spaceMap).map(([id, url]) => [url, Number.parseInt(id)]),
//   );

// export const getSpaceReverseMap = async (
//   client: DGSupabaseClient,
//   spaceIds?: number[],
...
//     throw error;
//   }
//   return Object.fromEntries(data.map(({ id, url }) => [url!, id!]));
// };
...
export const dbNodeSchemaToCrossApp = (
  ...
  return {
    spaceUrl: spaceMap[schema.space_id],
...
export const dbRelationTripleSchemaToCrossApp = ({
  ...
  const relation = conceptMap[references["relation_type"] ?? 0];
  const sourceType = conceptMap[references["source"] ?? 0];
  const destinationType = conceptMap[references["source"] ?? 0];
...
export const dbRelationToCrossApp = ({
  ...
  const references = (relation.reference_content ?? {}) as Record<
    ...
    number
  >;
  const relationType = conceptMap[references["schema_id"] || 0];
  if (relationType === undefined) throw new Error("Missing relationType");
  const source = conceptMap[references["source"] || 0];
  if (source === undefined) throw new Error("Missing source");
  const destination = conceptMap[references["destination"] || 0];
  if (destination === undefined) throw new Error("Missing destination");

  return {
    spaceUrl: spaceMap[relation.space_id],
...
export const dbRelationsToCrossApp = async ({
  ...
  if (spaceMap === undefined) {
    const spaceIds = relations.map((r) => r.space_id);
    spaceMap = await getSpaceMap(client, [...new Set(spaceIds)]);
  }
  if (conceptMap === undefined) {
    ...
    conceptMap = await getConceptMap({
      client,
      conceptIds: [...new Set([...schemaIds, ...nodeIds])],
      spaceId,
      spaceMap,
    });
  }
