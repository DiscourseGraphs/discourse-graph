...
/**
 * Ensure nodeInstanceId exists in frontmatter, creating it if missing
 */
const ensureNodeInstanceId = async (
  ...
  // Generate UUID and add to frontmatter
...
const getAllDiscourseNodesSince = async ({
  ...
  // First pass: collect all discourse nodes and ensure nodeInstanceId exists
  ...
  // Fetch existing content from database to check for filename changes
  ...
  if (error) {
    ...
    // Continue anyway - we'll just check mtime
  }

  // Create a map of nodeInstanceId -> existing text
  ...
  // Second pass: check if node should be synced
  for (const candidate of candidateNodes) {
    ...
    // Check if filename changed (even if mtime didn't change)
    const filenameChanged =
      existingText !== undefined && existingText !== currentFilename;
    // Check if this is a new file (not in database yet)
...
/**
 * Main sync function - uploads discourse nodes to Supabase
 */
...
/**
 * Convert discourse nodes to Supabase concepts and upsert them
 */
