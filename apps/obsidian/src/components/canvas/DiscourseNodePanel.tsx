...
export const DiscourseNodePanel = ({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (e: PointerEvent) => {
      ...
      const panelEl = rPanelContainer.current;
      if (!panelEl) return;

      const editorContainer = panelEl.closest(".tldraw__editor");
    ...
    document.addEventListener("keydown", handleKeyDown);
    const panelEl = rPanelContainer.current;
    const editorContainer = panelEl?.closest(
      ".tldraw__editor",
    ) as HTMLElement | null;
    const previousCursor = editorContainer?.style.cursor;
    if (editorContainer) editorContainer.style.cursor = "copy";
    return () => {
      document.removeEventListener("pointerdown", handlePointerDown, true);
      document.removeEventListener("keydown", handleKeyDown);
      if (editorContainer) editorContainer.style.cursor = previousCursor || "";
    };
  }, [focusedNodeTypeId, editor, plugin, canvasFile]);
