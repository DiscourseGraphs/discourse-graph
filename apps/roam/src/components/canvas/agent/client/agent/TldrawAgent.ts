...
export class TldrawAgent {
  ...
  dispose() {
    ...
    $agentsAtom.update(this.editor, (agents) =>
      agents.filter((agent) => agent.id !== this.id),
    );
  }
  ...
  async preparePrompt(
    ...
    return Object.fromEntries(
      transformedParts.map((part) => [part.type, part]),
    ) as AgentPrompt;
  }
      ...
      for await (const action of streamAgent({ prompt, signal })) {
        ...
        editor.run(
          () => {
            ...
            const transformedAction = actionUtil.sanitizeAction(
              action,
              helpers,
            );
