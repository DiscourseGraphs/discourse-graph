...
export class AgentHelpers {
  ...
  ensureShapeIdIsUnique(id: string): string {
    ...
    // Be defensive in case an undefined/non-string id slips through
    let idWithoutPrefix: string;
    const hadPrefix = typeof id === "string" && id.startsWith("shape:");
    if (!id || typeof id !== "string") {
      idWithoutPrefix = `shape_${Date.now()}_${Math.random()
        .toString(36)
        .slice(2, 9)}`;
    } else {
      idWithoutPrefix = hadPrefix ? id.slice(6) : id;
    }
    ...
    // Return the same prefix style as the input: if it had a 'shape:' prefix, keep it; otherwise return plain
    return hadPrefix ? `shape:${newId}` : newId;
  }
