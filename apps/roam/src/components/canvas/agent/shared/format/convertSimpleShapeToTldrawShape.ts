...
export function convertSimpleShapeToTldrawShape(
  ...
  switch (simpleShape._type) {
    case "text": {
      return convertTextShapeToTldrawShape(editor, simpleShape, {
        defaultShape,
      });
    }
    case "line": {
      return convertLineShapeToTldrawShape(editor, simpleShape, {
        defaultShape,
      });
    }
    case "arrow": {
      return convertArrowShapeToTldrawShape(editor, simpleShape, {
        defaultShape,
      });
    }
    case "cloud":
    ...
    case "heart":
    case "ellipse": {
      return convertGeoShapeToTldrawShape(editor, simpleShape, {
        defaultShape,
      });
    }
    case "note": {
      return convertNoteShapeToTldrawShape(editor, simpleShape, {
        defaultShape,
      });
    }
    case "draw": {
      return convertDrawShapeToTldrawShape(editor, simpleShape, {
        defaultShape,
      });
    }
    case "unknown": {
      return convertUnknownShapeToTldrawShape(editor, simpleShape, {
        defaultShape,
      });
    }
  }
...
export function convertSimpleTypeToTldrawType(
  ...
  if (type in SIMPLE_TO_GEO_TYPES) {
    return convertSimpleGeoTypeToTldrawGeoGeoType(
      type as SimpleGeoShapeType,
...
export const SIMPLE_TO_GEO_TYPES: Record<
  SimpleGeoShapeType,
  TLGeoShapeGeoStyle
> = {
  rectangle: "rectangle",
...
function convertTextShapeToTldrawShape(
  ...
  const autoSize =
    simpleShape.wrap === undefined
      ? (defaultTextShape.props?.autoSize ?? true)
      : !simpleShape.wrap;
  const textFontSize = FONT_SIZES[textSize];
  const textAlign =
    simpleShape.textAlign ?? defaultTextShape.props?.textAlign ?? "start";
  ...
  return {
    shape: {
      ...
      props: {
        ...
        color: asColor(
          simpleShape.color ?? defaultTextShape.props?.color ?? "black",
        ),
        textAlign,
        autoSize,
        w: measurement.w,
  ...
  return {
    shape: {
      ...
      props: {
        ...
        color: asColor(
          simpleShape.color ?? defaultLineShape.props?.color ?? "black",
        ),
...
function convertArrowShapeToTldrawShape(
  ...
  const shape = {
    ...
    props: {
      ...
      color: asColor(
        simpleShape.color ?? defaultArrowShape.props?.color ?? "black",
      ),
  ...
  if (simpleShape.fromId) {
    ...
    if (startShape) {
      ...
      const finalNormalizedAnchor = calculateArrowBindingAnchor(
        editor,
        startShape,
        targetPoint,
      );
...
function convertGeoShapeToTldrawShape(
  ...
  return {
    shape: {
      ...
      props: {
        align:
          simpleShape.textAlign ?? defaultGeoShape.props?.align ?? "middle",
        color: asColor(
          simpleShape.color ?? defaultGeoShape.props?.color ?? "black",
        ),
...
function convertNoteShapeToTldrawShape(
  ...
  return {
    shape: {
      ...
      props: {
        color: asColor(
          simpleShape.color ?? defaultNoteShape.props?.color ?? "black",
        ),
        richText,
...
function convertDrawShapeToTldrawShape(
  ...
  return {
    shape: {
      ...
      props: {
        color: asColor(
          simpleShape.color ?? defaultDrawShape.props?.color ?? "black",
        ),
        fill,
      },
      meta: {
