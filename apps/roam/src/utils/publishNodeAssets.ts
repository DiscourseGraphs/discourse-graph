...
export const publishNodeAssets = async ({
  ...
  // entirely: nothing copied for it, and nothing removed from it. A node that is here
  // with no references is a different case, and still gets its cleanup below.
  const nodesWithContent = nodes.flatMap((node) => {
    const markdown = node.content.full?.value;
    return markdown === undefined
      ? []
      : [{ node, referenced: [...new Set(findAssetReferences(markdown))] }];
  });
  if (nodesWithContent.length === 0) return [];
  ...
  try {
    existingByNode = await readExistingReferences({
      client,
      spaceId,
      sourceLocalIds: nodesWithContent.map(({ node }) => node.localId),
    });
  } catch (error) {
    ...
    return nodesWithContent.flatMap(({ node, referenced }) =>
      referenced.map(
  ...
  for (const { node, referenced } of nodesWithContent) {
    const existing =
