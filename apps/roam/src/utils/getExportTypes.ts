...
const getExportTypes = ({
  ...
  const settings = settingsSnapshot ?? bulkReadSettings(trace);
  const allRelations = getDiscourseRelations(settings, trace);
  const allNodes = getDiscourseNodes(allRelations, settings, trace);
