...
// The real one reports through posthog and sends an email: `NODE_ENV` is "test" here,
// not "development", so it takes the reporting branch.
...
describe("asset registry", () => {
  ...
  it("ignores a block on the page that is not the registry block", () => {
    // A page with blocks but no registry block is the orphan condition, so this
    // reports. Asserted in the next test rather than here.
    graph.pages.set(ASSET_REGISTRY_PAGE_TITLE, [
  ...
  it("warns once when the page outlives its registry block, however many lookups follow", async () => {
    ...
    expect(mockedInternalError).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        error: expect.stringContaining(ASSET_REGISTRY_BLOCK_TEXT) as unknown,
      }),
    );
  });
