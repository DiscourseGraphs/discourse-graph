...
const makeFakeClient = ({
  ...
  // `eq` chains and is awaitable on its own, because the asset stage reads a node's
  // existing references with two `eq`s and no `in`.
  const selectFilter = (table: string): Record<string, unknown> => ({
    eq: () => selectFilter(table),
    in: () => selectResult(table),
    then: (resolve: (value: unknown) => unknown) =>
      selectResult(table).then(resolve),
  });
