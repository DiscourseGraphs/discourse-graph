...
async function getOrCreateDiscourseSpace(
  ...
  if (!name || !url || platformId === undefined || platformId === null) {
    return {
      error: "DiscourseSpace name, URL, and platform are required",
      space: null,
      created: false,
    };
  }
  ...
  const { data: existingSpace, error: fetchError } = await supabase
    .from("Space")
    .select("id, name, url, platform_id")
  ...
  if (fetchError) {
    console.error(
      `Error fetching DiscourseSpace (URL: ${normalizedUrl}, PlatformID: ${platformId}):`,
      fetchError,
    );
    return {
  ...
  if (existingSpace) {
    ...
    console.log(
      `DiscourseSpace "${name}" (URL: ${normalizedUrl}, PlatformID: ${platformId}) not found, creating new one...`,
    );
    ...
    if (insertError) {
      console.error(
        `Error inserting new DiscourseSpace (Name: ${name}, URL: ${normalizedUrl}, PlatformID: ${platformId}):`,
        insertError,
      );
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      response = NextResponse.json(
        { error: "Missing or invalid platform in request body" },
        { status: 400 },
      );
      return cors(request, response) as NextResponse;
    }
    ...
    if (error) {
      console.error(
        `API Error for DiscourseSpace (Name: ${name}, URL: ${url}, PlatformID: ${platform}): ${error}`,
        details || "",
      );
      ...
      if (space) {
        ...
        console.error(
          `API Error for DiscourseSpace (Name: ${name}, URL: ${url}, PlatformID: ${platform}): Space was null without an error flag.`,
        );
        response = NextResponse.json(
          {
            error:
